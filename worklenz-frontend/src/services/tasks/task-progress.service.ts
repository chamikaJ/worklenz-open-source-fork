...
import { updateTaskProgress, setManualProgress, updateSubtaskProgress } from '@/features/tasks/tasks.slice';
...
class TaskProgressService {
   ...
   * @param progressValue - The progress value to set (0-100)
  ...
  public setManualProgress(
    ...
    return new Promise((resolve) => {
      ...
      try {
        this.socket.emit(
          SocketEvents.SET_MANUAL_PROGRESS.toString(),
          {
            task_id: taskId,
            enable_manual: enableManual,
            progress_value: Math.round(progressValue),
            team_id: teamId
          }
        );
  ...
  private handleTaskProgressUpdate = (data: TaskProgressInfo): void => {
    ...
    if (taskId) {
      ...
      store.dispatch(updateBoardTaskProgress({
        id: data.id,
      ...
      if (data.parent_task) {
        store.dispatch(updateSubtaskProgress({
          subtaskId: data.id,
  ...
  private handleManualProgressUpdate = (data: {
    ...
    if (data?.success) {
      store.dispatch(setManualProgress({
        taskId: data.task_id,
